...
export default function ServiciosPage() {
  ...
  {loading ? <Loader /> : items.length===0 ? <EmptyState message="No hay servicios registrados." /> :
    ...
    <table className="min-w-full text-sm">
      ...
      <tbody>{items.map(i => <tr key={i.id} className="border-t border-slate-200">
      <td className="px-4 py-3">{i.codigo}</td>
      <td className="px-4 py-3 font-medium">{i.nombre}</td>
      <td className="px-4 py-3">{i.descripcion || "-"}</td>
      {/* <td className="px-4 py-3">{formatCurrency(i.precioBase)}</td> */}

      </tr>)}</tbody>
      </table>
      </TableWrapper>}

  <FormModal open={open} title="Nuevo servicio" onClose={()=>setOpen(false)}>
    <form onSubmit={save} className="grid gap-4 md:grid-cols-2">
      ...
      <textarea className="input md:col-span-2" placeholder="Descripción" value={form.descripcion} onChange={e=>setForm({ ...form, descripcion:e.target.value })} />
      <input className="input" type="number" step="0.01" placeholder="Precio base" value={form.precioBase} onChange={e=>setForm({ ...form, precioBase:Number(e.target.value) })} />
      <div className="md:col-span-2 flex justify-end"><button className="btn-primary" type="submit">Guardar</button>
     </div></form></FormModal></div>;
}
