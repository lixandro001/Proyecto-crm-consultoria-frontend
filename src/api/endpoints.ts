...
export const contratosApi = { list: async () => (await api.get<ApiResponse<Contrato[]>>("/contratos")).data, create: async (p:any)=> (await api.post("/contratos", p)).data, downloadWord: async (id:number)=> await api.get(`/contratos/descargar-word/${id}`, { responseType:"blob" }), generarPdf: async (contratoId:number)=> (await api.post("/contratos/generar-pdf", { contratoId })).data };
